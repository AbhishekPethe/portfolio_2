...
const SoundBar = () => {
    ...
    return (
        <motion.div
            className="flex cursor-none gap-1 absolute top-[6%] left-[50%] translate-x-[-50%] max-[500px]:translate-y-[-100%] invisible
            md:visible lg:translate-y-[-30%] z-10  "
            onClick={handleClick}
            onMouseEnter={() => {
        ...
        >
            {[0.2, 0.3, 0.4, 0.5, 0.8].map((delay, index) => (
                <span
                    key={index}
                    className={`bg-white border border-current h-4 w-1 rounded-xl mx-px
                    ${click ? 'animate-play' : ''}`}
